...
const MultiSelect = ({ onChange, data }: MultiSelectProps) => {
  const inputRef = React.useRef<HTMLInputElement>(null);
  const [open, setOpen] = React.useState(false);
  const [selected, setSelected] = React.useState<DataItem[]>([]);
  const [inputValue, setInputValue] = React.useState("");

  const handleUnselect = React.useCallback((data: DataItem) => {
  ...
  // const selectables = data.filter((data) => {
  //   console.log(!selected.includes(data));
  ...
  const selectables = useMemo(
    () =>
      data.filter(
        (d) => !selected.map((select) => select.value).includes(d.value)
      ),
    [data, selected]
  );
